...
class SQLiteDatabase(private val app: Application) {
    ...
    fun getList(ids: List<String>, language: String): List<CardInfo> {
        val table = getTextTable(language)
    ...
    fun find(key: String, isEffect: Boolean, lang: String): List<CardInfo> = find(key, lang, if (isEffect) "desc" else "name")

    private fun find(key: String, language: String, field: String): List<CardInfo> {
        val table = getTextTable(language)
        val list = mutableListOf<CardInfo>()
